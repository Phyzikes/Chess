...
void move() {
	...
	if (moveState == "black") cout << endl << "Please, wait! Black is thinking ..." << endl;
};
// Рокировка
void castling() {
...
void enterMove() {
	if (moveState == "white") {
		...
		// Генерация случайных чисел от 0 до 7 для ИИ ("случайный ход")
		bool goNext = false;
		do {
			srand(static_cast<unsigned int>(time(0)));
			int m = (rand() % 8);
			int n = (rand() % 8);
			if (desk[m][n].second[0] == 'B') {
				figure = desk[m][n].second;
				fromTo = desk[m][n].first;
				goNext = true;
			}
		} while (!goNext);
		for (int t = 0; t < 8; t++) {
			for (int k = 0; k < 8; k++) {
				if (validMove(figure, fromTo, desk[t][k].first)) {
					whereTo = desk[t][k].first;
					break;
				}
			}
		}
